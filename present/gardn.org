...
#+END_SRC
  ...
  - Write, compile, run, read, debug, edit ...
...
* The programmer cycle
  ...
  - Java, Scala, Python, ...
    ...
    - Configure with some other language.

  - Clojure

    - Run, write, read, edit, read, ...
    - Work within the system as you build it.
    - Configure with the language you build in.
...
#+BEGIN_SRC clojure
  [... {:db/id 9
       ...
       {:db/id 444
        ...
        :album/tracks [5050 ...] ...}
       {:db/id 5050
        :track/name    "Tomorrow Never Knows"
        :track/artists [23] ...} ...]
#+END_SRC
...
#+BEGIN_SRC clojure
  ...
  (def squares  (map (fn [x] (* x x)) whole))
